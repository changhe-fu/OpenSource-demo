...
KindEditor.plugin("quickformat", function (K) {
  ...
  self.clickToolbar(name, function () {
    ...
    while (child) {
      ...
      if (!firstChild || (firstChild.name != "img" && firstChild.name != "video")) {
        if (blockMap[child.name]) {
          child.html(child.html().replace(/^(\s|&nbsp;|　)+/gi, ""));
          child.attr(
            "style",
            "text-align:justify;text-indent:2em;font-size:16px;font-family:Microsoft YaHei;line-height: 2;margin-bottom:24px;margin-top:16px;color:#333;"
          );
        } else {
          if (child.name == "img") {
            child.attr("style", "max-width:750px;display:block;margin:0 auto;");
          }
          subList.push(child);
        }
        if (
      ...
      } else if (firstChild.name == "img" || firstChild.name == "video") {
        firstChild.attr(
          "style",
          "max-width:100%;display:block;margin:0 auto;"
        );
        subList.push(firstChild);
      }
      child = next;
    }
    K.each(nodeList, function (i, subList) {
      var wrapper = K(
        '<p style="text-indent:2em;font-size:16px;font-family:Microsoft YaHei;line-height: 28px;margin-bottom:24px;margin-top:16px;letter-spacing:2px;color:#333;"></p>',
        doc
      );
      if (subList[0].name == "#text") {
